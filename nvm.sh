...
nvm_ensure_posix_path() {
  ...
  [ "$(command printf '%s' "$location" | command cut -b 1)" = "/" ] && nvm_echo "$location" && return 0

  # If absolute path (eg. starting with C:)
  if command printf '%s' "$location" | command grep '^[a-zA-Z]:' >/dev/null 2>&1; then
    # Get lower case drive letter, eg. c for C:\...
    local letter
    letter=$(command printf '%s' "$location" | command cut -b 1 | command tr '[:upper:]' '[:lower:]')
    # Get upper case drive letter, eg. C for C:\...
    local letter_upper
    letter_upper=$(command printf '%s' "$letter" | command tr '[:lower:]' '[:upper:]')
    local letter_prefix
    if [ -d '/mnt/' ]; then
      letter_prefix='/mnt'
    elif [ -d '/drives/' ]; then
      letter_prefix='/drives'
    fi

    # prefix like /mnt/c, /drives/c or /c
    ...
    # prefix like /mnt/C, /drives/C or /C
      ...
      # Not possible to unit test properly this case
    ...
    # Set prefix
    location="${letter_prefix}${location}"
  fi
